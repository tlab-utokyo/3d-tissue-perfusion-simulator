...
export function Metrics({ params, metrics: m, zeroOrderNecrosisRadius }: Props) {
  const reactionTex =
    params.reaction === "mm"
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.cols}>
        ...
        {/* 4. ゼロ次参照 */}
        <section className={styles.col}>
          <div className={styles.colLabel}>ゼロ次参照（同一BC）</div>
          <div className={styles.grid}>
            <div className={styles.cell}>
          ...
          <div className={styles.note}>
            Da &gt; 2 は平板片側灌流での壊死コア発生の目安。壊死前縁は
            {params.species === "O2"
              ? " 壊死閾値 C_necrosis（編集可・既定 0.003 mM≈2.3 mmHg）"
              : " 枯渇閾値 0.1·K_m"}
            で判定。点線「ゼロ次参照」は同一BCのゼロ次定常で、MM の数値指標とは別物。
          </div>
        </section>
      </div>
    </div>
